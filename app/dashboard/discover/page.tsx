...
export default function DashboardDiscoverPage() {
  ...
  const handlePreviewClick = async (track: SpotifyTrack) => {
    ...
    const res = await fetch(
      `/api/deezer/preview?track=${encodeURIComponent(track.name)}&artist=${encodeURIComponent(track.artists[0]?.name ?? "")}`
    );
      ...
      {showPlaylistModal ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[rgba(13,13,18,0.74)] px-4 backdrop-blur-sm">
          <div className="glass-panel w-full max-w-md rounded-[28px] p-6">
            ...
            {loadingPlaylists ? (
              <p className="mt-6 text-sm text-[var(--color-muted)]">Loading playlists...</p>
            ) : (
              <select
